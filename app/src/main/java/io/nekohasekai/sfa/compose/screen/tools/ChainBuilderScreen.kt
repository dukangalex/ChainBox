...
fun ChainBuilderScreen(navController: NavController) {
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    for (i in 0 until outs.length()) {
                        ...
                        if (localIdx >= 0) {
                            // Prefer group tag if saved local was a leaf
                    ...
                    loadError = when {
                        ...
                        currentMainTag == null -> "当前配置找不到可用分组（需要「节点选择」或「自动选择」等 selector/urltest）"
                        else -> null
                    }
                } catch (e: Exception) {
    ...
    fun save() {
        ...
        if (main.isNullOrBlank()) {
            scope.launch { snackbar.showSnackbar("当前配置没有可用分组（节点选择/自动选择）") }
            return
        }
                ...
                Column(Modifier.padding(12.dp)) {
                    ...
                    Text(
                        text = when {
                            ...
                            else -> "当前侧优先用「节点选择/自动选择」分组，不会锁死某个单节点。"
                        },
                        style = MaterialTheme.typography.bodySmall,
...
private fun resolveCurrentMainTag(outs: JSONArray, routeFinal: String): String? {
    // Prefer user's group (selector/urltest), never freeze to a random leaf node.
    val preferNames = listOf("节点选择", "自动选择", "手动切换", "proxy", "Proxy", "GLOBAL")
    data class Cand(val tag: String, val type: String, val score: Int)
    ...
    for (i in 0 until outs.length()) {
        ...
        var score = 10
        when {
            tag in preferNames -> score = 100
            tag.contains("节点") || tag.contains("选择") -> score = 90
            type == "urltest" && (tag.contains("自动") || tag.equals("auto", true)) -> score = 95
            type == "urltest" -> score = 80
            type == "selector" -> score = 70
        }
        if (routeFinal == tag) score += 5
        cands.add(Cand(tag, type, score))
    }
    cands.maxByOrNull { it.score }?.let { return it.tag }
...
private fun mergeHopTree(outs: JSONArray, hop: HopRef, sourcePath: String): JSONArray {
    ...
    if (hop.isGroup) {
        ...
        for (i in 0 until members.length()) {
            ...
            // Skip nested groups (selector/urltest as member) — leaf proxies only
            val mType = mSrc.optString("type")
            if (mType == "selector" || mType == "urltest" || mType == "chain") continue
