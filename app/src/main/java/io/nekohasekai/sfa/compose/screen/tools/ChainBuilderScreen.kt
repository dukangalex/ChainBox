        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    loadError = null
                    val enabled = Settings.chainEnabled || routeFinal.startsWith("ext-")
                    chainActive = enabled
                    if (enabled && Settings.chainLandingProfileId >= 0L && Settings.chainLandingTag.isNotBlank()) {
                        val lid = Settings.chainLandingProfileId
                        val ltag = Settings.chainLandingTag
                        val found = others.flatMap { it.hops }.find {
                            it.profileId == lid && it.tag == ltag
                        }
                        if (found != null) {
                            exit = found
                            savedHint = "链路：当前「${current.name}」(${main ?: "?"}) → ${found.displayLine}"
                        } else {
                            val pName = profiles.find { it.id == lid }?.name ?: "落地"
                            exit = HopRef(lid, pName, ltag, "selector")
                            savedHint = "链路：当前「${current.name}」(${main ?: "?"}) → $pName / $ltag（已固定）"
                        }
                    } else if (!enabled) {
                        exit = null
                    }
        ...
        scope.launch {
            val result = withContext(Dispatchers.IO) {
                try {
                    ...
                    // 当前 → 落地 → 目标
                    applyDetourToLeaves(outs, main, landing.mergedTag)
                    root.put("outbounds", outs)
                    ...
                    LazyColumn(modifier = Modifier.heightIn(max = 360.dp)) {
                        items(filtered) { hop ->
                            Column(
                                ...
                                Text(hop.profileName, style = MaterialTheme.typography.bodySmall)
