...
设置 → 应用版本 应与 Release tag 一致（如 1.0.16）。
...
订阅更新后不必重新点选，只要曾经为这份配置保存过。入口/落地分组被订阅改名后，到链式页重新选择即可。

**取消链式**：只取消当前配置；其他配置的绑定保留。
...
WebDAV（含 TeraCLOUD）走 HTTPS。若 VPN 已开启，备份会尽量改走系统 Wi-Fi/蜂窝直连，避免证书被隧道干扰。连通性测试失败时，仍以实际上传/下载是否成功为准。
