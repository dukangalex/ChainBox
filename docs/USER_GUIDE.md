...
路径：**工具 → 脚本**。可导入多条 `function main(config)` 脚本（代码 / URL / 文件），并单独开关。脚本只改**运行时** sing-box JSON，不写回订阅。Clash 字段（`proxy-groups`、`rule-providers`、`mixed-port` 等）不会被识别。内置开关（中国直连、广告拦截、防 WebRTC 等）在脚本之后仍然生效。可导入自带的「机场地区分组」示例：按节点名生成地区 urltest/selector、补充 DNS / 嗅探 / mixed 入站和远程规则集。
